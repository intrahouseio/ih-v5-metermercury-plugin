...
function getOpenReq(password) {
  if (!password) {
    password = Buffer.from([1, 1, 1, 1, 1, 1]); // 0x01,0x01,..; 
  } else {
...
function read3ByteValueArr(buf, pollItem, assets) {
  if (pollItem.arr.length == 4 && buf.length < 12) return [];
  if (pollItem.arr.length == 3 && buf.length < 9) return [];

  const res = pollItem.arr.map((item, index) => {
    const int32Buf = Buffer.from([0, 0, 0, 0]);
    int32Buf[1] = pollItem.mid == 'I' || pollItem.mid == 'U' ? buf[index*3] : buf[index*3] & 0x3f; // Маскировать 2 старших бита
    int32Buf[2] = buf[index*3 + 2];
    int32Buf[3] = buf[index*3 + 1];
    return { chan: pollItem.mid + item, value: calculate3ByteValue(pollItem.mid, int32Buf.readUInt32BE(0), assets) };
  })
...
function read4Energy(buf, pollItem, assets) {
  ...
  const buffer = Buffer.alloc(16,buf).swap16();
  
  res.push(buffer.readUInt32BE(0));
  res.push(buffer.readUInt32BE(4));
  res.push(buffer.readUInt32BE(8));
  res.push(buffer.readUInt32BE(12));

  return [
    { chan: 'EAP' + sut + tstr, value: res[0] == 4294967295 ? '' : round(res[0]*0.001) },
    { chan: 'EAM' + sut + tstr, value: res[1] == 4294967295 ? '' : round(res[1]*0.001) },
    { chan: 'ERP' + sut + tstr, value: res[2] == 4294967295 ? '' : round(res[2]*0.001) },
    { chan: 'ERM' + sut + tstr, value: res[3] == 4294967295 ? '' : round(res[3]*0.001) }
  ];
}
...
function getPollItems(mid, period, ntariffs) {
  switch (mid) {
    ...
    */
    ...
    case 'Q':
    case 'S':
      return getReq16(mid, period, ['0', '1', '2', '3']);
    case 'cos':
    case 'Kuf':
      return getReq14(mid, period, ['0', '1', '2', '3']);
...
function getReq14(mid, count, arr) {
  return [
    {
      mid,
      buf: Buffer.from([0, 0x8, 0x11, getRWRI(mid, 0), 0, 0]),
      arr,
...
function getReq16(mid, count, arr) {
  return [
    {
      mid,
      buf: Buffer.from([0, 0x8, 0x16, getRWRI(mid, 0), 0, 0]),
      arr,
      readfn: read3ByteValueArr,
      count
    }
