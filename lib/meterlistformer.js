  ...
  try {
    ...
    devhard.forEach(item => {
      if (item.foldertype == 'node') {
        if (!item.addr) {
          plugin.log(item.chan + ': отсутствует серийный номер счетчика. Не включается в список!', 1);
        } else {
    ...
    devhard.forEach(item => {
      if (!item.folder) {
        if (!meterMap[item.parent]) {
          plugin.log('WARN: В списке счетчиков отсутствует узел для канала: ' + util.inspect(item) + '. Пропускается..', 1);
    ...
    plugin.log('ERROR: meterlistformer ' + util.inspect(e), 1);
    return [];
  }
    ...
    for (let item of chanArr) {
      ...
      if (pollArrayIdx >= 0) {
        ...
        if (pollArrayIdx >= 0) {
          ...
          plugin.log('WARN: Для канала ' + chan + ' не найдены правила опроса! Канал не будет опрошен', 1);
        }
      }
    }
    return res;
